# telegram_bot.py - Telegram version of Instagram Unban Monitor
# All features same as Discord version, adapted for Telegram.

"""
Instagram Unban Monitor — Telegram Bot
---------------------------------------
Tracks Instagram accounts you flag as "banned" and posts a Telegram
notification with full stats the moment they become reachable again.

- Only the bot owner (user ID set in OWNER_USER_ID) can use commands.
...
Commands (owner only):
  ...
  /trackpost url         - start monitoring a specific post/reel/story for removal
  /untrackpost url       - stop monitoring one
  /postbanlist           - show everything monitored for post/story removal
  /checkpost url         - debug: immediately check if a post/story is still up
  /setchannel            - set the current chat as the notification destination
  /checknow username     - debug: immediately check one account

Setup: set environment variables (see README)
"""
...
OWNER_USER_ID = int(os.getenv("OWNER_USER_ID", "0"))  # Your Telegram user ID
...
# ---------- D1 storage (same as Discord version) ----------
def _d1_headers():
    return {
...
# ---------- Rate limiting / notification queue ----------
notification_queue = deque()
is_processing_notifications = False

async def send_telegram_message(chat_id: int, text: str, parse_mode: str = ParseMode.MARKDOWN) -> bool:
...
# ---------- Instagram status check (unchanged from Discord version) ----------
...
async def check_tracked_accounts():
    try:
        ...
        # Filter to only active items (skip already finished)
        active_items = {}
        ...
        print(f"🔍 Checking {pending_count} active item(s)...", flush=True)
        if pending_count == 0:
            return
            ...
            if track_type == "post":
                ...
                if result is not None and result.get("available") is False:
                    ...
                    if fail_count >= 2:
                        ...
                        await queue_notification(chat_id, text, username)
...
async def background_check_loop():
    """Run the check loop every CHECK_INTERVAL_MINUTES."""
    while True:
        await check_tracked_accounts()
...
def is_owner(update: Update) -> bool:
    ...
    if not update.effective_user:
        return False
    return update.effective_user.id == OWNER_USER_ID
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Instagram Unban Monitor (Telegram)\n"
...
async def trackpost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Do initial check to save stats
...
def main():
    ...
    if not OWNER_USER_ID:
        raise SystemExit("OWNER_USER_ID not set.")
    ...
    loop.create_task(background_check_loop())
