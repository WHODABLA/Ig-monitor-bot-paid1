# telegram_bot.py - FINAL WORKING VERSION (LOOP FIXED)
...
def main():
    ...
    # Clear webhook to avoid conflicts - KEEP THE LOOP ALIVE
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(application.bot.delete_webhook(drop_pending_updates=True))
    # DO NOT CLOSE THE LOOP - it will be reused by run_polling
    # loop.close()  # <-- REMOVED
    time.sleep(2)
    ...
    if application.job_queue:
        ...
        # Fallback: schedule the task on the same loop
        loop = asyncio.get_event_loop()
        loop.create_task(check_tracked_accounts())

    # Start polling - this will use the existing loop
